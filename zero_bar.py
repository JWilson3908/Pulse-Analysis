...
class timetotemp:
    ...
    def __init__(self,*nums):
        self.tc=[2.4292,0.929,1.013,2.293] # list of Tc for experiments
        self.set=nums[0]
        self.num_exp=nums[1]
        self.num1=nums[2]
        self.num2=nums[3]
        ...
        self.donecutting=nums[5]
        
        if self.set is 0: # zero bar
            self.dir="C:\\Users\\John Wilson\\Documents\\Cornell REU\\Data\\All Data\\" # home dir 0 Bar
#            self.dir="c:\\Users\\JMP\\Documents\\Thermal Conductivity\\Backup\\2018FEB\\" # work dir
            # Fork 1
            self.path1=[self.dir+"20180619\\CF1p0mK.dat",self.dir+"20180620\\CF2p1mK.dat"]
            # Fork 2
            self.path2=[self.dir+"20180619\\FF1p0mK.dat",self.dir+"20180620\\FF2p1mK.dat"]
            self.rawdata1,self.rawdata2=self.import_fun(self.path1,self.path2) # import fork1, fork 2
            
            n1s, n2s = [np.float64( range( 1, len( self.rawdata1[0] ) + 1 ) )], [np.float64( range( 1, len( self.rawdata2[0] ) + 1 ) )]
            
            self.rawdata1, self.rawdata2 = np.concatenate( ( self.rawdata1, n1s), 0 ), np.concatenate( ( self.rawdata2, n2s), 0 )
            
# We don't need pulse indices for non-pulsing data
...
#        self.calibration()
    ...
    def pulse_remove(self,n1,n2):
        ...
        if len(B.pulseID) != 0:
            ...
            d1=np.isin(range(0,len(self.rawdata1[1])),pulse_rem,assume_unique=True,invert = True)
            d2=np.isin(range(0,len(self.rawdata2[1])),pulse_rem,assume_unique=True,invert = True)
    ...
    def temp_fit(self,nump):
        ...
        elif self.donecutting == 0 :
            fig1 = plt.figure(7, clear = True)
            ax1 = fig1.add_subplot(111)
            ax1.set_ylabel('T')
            ax1.set_xlabel('Point Number')
            ax1.set_title('T and Point Number')
        
            ax1.plot(self.rawdata1[4][d1], self.rawdata1[2][d1], color='green',lw=1)
            
            ax1.plot(self.rawdata1[4][d1], fit_fn(self.rawdata1[0][d1]), color='blue',lw=1)
...
B = timetotemp(0,10,14400,28000,0,1) #425 psi
